...
import {
  Card, Tabla, Select, Cargando, Aviso, Badge, Boton, Modal, Campo, Input, FiltroChips,
} from '../../components/ui';
...
const FORM_VACIO = {
  nombre: '', categoria: 'Papel', unidad_medida: 'pieza', costo_referencia: '',
  proveedor_id: '', stock_minimo_default: '0', activo: true,
};
...
export default function Insumos() {
  ...
  async function cargar() {
    const [i, p] = await Promise.all([
      supabase.from('limpieza_insumos')
        .select('id, nombre, categoria, unidad_medida, costo_referencia, proveedor_id, stock_minimo_default, activo')
        .order('nombre'),
  ...
  function abrirEditar(i) {
    setEditando(i);
    setForm({
      nombre: i.nombre, categoria: i.categoria ?? 'Otro', unidad_medida: i.unidad_medida,
  ...
  async function guardar(e) {
    ...
    setGuardando(true);
    const payload = {
      nombre: form.nombre.trim(), categoria: form.categoria || null, unidad_medida: form.unidad_medida,
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm" style={{ color: 'var(--text-secondary)' }}>
            Catálogo de productos de limpieza — {d.insumos.length} registrados.
          </p>
        </div>
      ...
      <Card>
        <Tabla
          ...
          columnas={[
            ...
            { key: 'costo', header: 'Costo ref.', align: 'right', render: (i) => i.costo_referencia ? money(i.costo_referencia) : '—' },
      ...
      <Modal abierto={modal} onClose={() => setModal(false)} titulo={editando ? 'Editar insumo' : 'Nuevo insumo'}>
